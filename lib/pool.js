...
function newBlockTemplate(template) {
    ...
    for (let id in connectedMiners) {
         ...
         console.log('Updating worker ' + miner.id + ' With new work at height: ' + template.height);
         if (miner.sharesSinceLastJob > 6)
             estimateBestDiff(miner);
...
function estimateBestDiff(miner) {
    let ideal_spm = 50;                     //! ideal sharepermin    50
    let std_deviation = 10;                 //! acceptable deviation +-10
    let miner_spm = miner.sharesPerMinute;  //! measured spm
    let miner_diff = miner.difficulty;      //! current diff

    if ((miner_spm < (ideal_spm - std_deviation)) ||
        (miner_spm > (ideal_spm + std_deviation))) {
         let ratio = miner_spm / ideal_spm;
         let new_diff = ratio * miner_diff;
         console.log('\x1b[33mPropose moving from ' + miner_diff + '->' + new_diff + '\x1b[0m');
         miner.proposeNewDiff = new_diff;
    }
}
...
Miner.prototype = {
    ...
    getJob: function () {
...

        {
            ...
            if (this.proposeNewDiff != 0) {
               let padded = Buffer.alloc(32);
               padded.fill(0);
               let diffBuff = baseDiff.div(this.proposeNewDiff).toBuffer();
               diffBuff.copy(padded, 32 - diffBuff.length);
               let buff = padded.slice(0, 32);
               target = buff.toString('hex');
               this.difficulty = this.proposeNewDiff;
               this.proposeNewDiff = 0;
            }
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, blockTemplate, pool) {
    ...
    log('info', logSystem, '\x1b[32mAccepted share at difficulty %d/%d from %s\x1b[0m', [miner.difficulty, baseDiff.div(bignum(shareDiff)), miner.login]);
}
...
function processShare(miner, job, blockTemplate, params, sendReply) {
    ...
    if (hashDiff.le(blockTarget)) {
        ...
        console.warn("\x1b[31mRejected low diff share of " + hashDiff.toString() + " from: " + miner.address + " ID: " +
            miner.identifier + " IP: " + miner.ipAddress + "\x1b[0m");
        return false;
    } else {
        recordShareData(miner, job, hashDiff.toString(), false, null, shareType);
    }
