...
global.config.isRandomX = global.config.isRandomX || false;
...
if (global.config.daemonType === 'bytecoin') {
    previousOffset = global.config.previousOffset || 3;
    offset = global.config.offset || 3;
}
...
function newBlockTemplate(template) {
    ...
    for (let connID in connectedMiners) {
        if (connectedMiners.hasOwnProperty(connID)) {
            let miner = connectedMiners[connID];
...
let VarDiff = (function () {
    let variance = global.config.poolServer.varDiff.variancePercent / 100 * global.config.poolServer.varDiff.targetTime;
    return {
        variance: variance,
...
function Miner(id, login, pass, ip, startingDiff, pushMessage, portType, port) {
    ...
    this.shareTimeBuffer.enq(global.config.poolServer.varDiff.retargetTime);
...
Miner.prototype = {
    retarget: function (now) {
        ...
        if (avg > VarDiff.tMax && this.difficulty > options.minDiff) {
            newDiff = options.retargetTime / avg * this.difficulty;
    ...
    setNewDiff: function (newDiff) {
        ...
        if (this.newDiff > options.maxDiff && !this.proxy) {
            this.newDiff = options.maxDiff;
        }
        if (this.difficulty === this.newDiff) {
            return;
        }
        if (this.newDiff < options.minDiff) {
            this.newDiff = options.minDiff;
        }
